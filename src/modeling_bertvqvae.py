...
class VQVAESum(nn.Module):
    def __init__(self, args):
        super(VQVAESum, self).__init__()
        self.args = args
        self.skip_VQ = args.skip_VQ

        # 1. encoder
        self.bert = Bert()

        self.max_len = args.max_pos
        ...
        # 2. multi heads
        self.ch_dim = args.ch_dim
        self.cnn_enc = nn.Conv1d(1, self.ch_dim, 3, padding=1)
        self.cnn_dec = nn.Conv1d(self.ch_dim, 1, 3, padding=1)
        # self.layer_norm = nn.LayerNorm(self.hidden_size, eps=1e-12)
        self.cnn_dec.weight = nn.Parameter(self.cnn_enc.weight.transpose(0, 1))

        # 3. VQ-VAE
        self.vq_vae = VectorQuantizerEMA(self.ch_dim, args.num_cluster, args.commitment_cost, args.VQVAE_decay, args.num_avg_sample)
        ...
        self.generator = nn.Sequential(nn.Linear(args.dec_hidden_size, self.vocab_size), nn.LogSoftmax(dim=-1))
    ...
    def forward(self, encoder_input_ids, decoder_input_ids, token_type_ids, encoder_attention_mask):    # decoder_attention_mask (not used)
        # encoder
        outputs = self.bert(input_ids=encoder_input_ids, token_type_ids=token_type_ids, attention_mask=encoder_attention_mask)
        batch_size = outputs[0].size(0)
        output, pooled, hidden = outputs

        if False:
            # sentence representation = avearge vector from second to last layer
            emb_vectors = hidden[-2]
            mask = encoder_attention_mask.unsqueeze(-1)
            emb_vectors = emb_vectors * mask.float()
            emb_vector = emb_vectors.sum(dim=1) / mask.sum(dim=1)
        else:
            # [CLS] embedding from top layer
            emb_vector = output[:, 0]

        # cnn encoder
        multi_vector = (self.cnn_enc(emb_vector.unsqueeze(1)))          # (B, 100, 768)
        multi_vector = multi_vector.permute(0, 2, 1).contiguous()       # (B, 768, 100)
        multi_vector = multi_vector.view(-1, self.ch_dim)               # (Bx768, 100)

        if self.skip_VQ:
            quantized = multi_vector
            vq_outputs = None
        else:
            vq_outputs = self.vq_vae(multi_vector)
            # loss, quantized, perplexity, encodings, distances = vq_outputs
            quantized = vq_outputs[1]

        # cnn decoder
        quantized = quantized.view(batch_size, -1, self.ch_dim).permute(0, 2, 1)
        q_vector = self.cnn_dec(quantized)                  # (B, 1, 768)

        # decoder
        q_sequence = q_vector.repeat(1, self.max_len, 1)
        dec_state = self.decoder.init_decoder_state(encoder_input_ids, q_sequence)
        decoder_outputs, _ = self.decoder(decoder_input_ids[:, :-1], q_sequence, dec_state)

        return (decoder_outputs, vq_outputs)


class Bert(nn.Module):
    def __init__(self, output_hidden_states=True, initial=True):
        super().__init__()
        self.model = BertModel.from_pretrained('bert-base-uncased', output_hidden_states=True, return_dict=False)  # , return_dict=False
...
class VectorQuantizerEMA(nn.Module):
    def __init__(self, embedding_dim, num_embeddings, commitment_cost, decay, num_sample=1, epsilon=1e-5):
    ...
    def forward(self, inputs):
        ...
        if True:
            ...
            # encoding_indices = torch.topk(-distances, self._num_sample, dim=1)[1]
            encodings = torch.zeros(input_shape[0], self._num_embeddings, device=inputs.device)
            encodings.scatter_(1, encoding_indices, 1)
        else:
            # sampling & encoding (B, K)
            dist_prob = 1. / distances
            dist_prob = dist_prob / dist_prob.sum(dim=1, keepdim=True)
            m = Multinomial(self._num_sample, dist_prob)
            encodings = m.sample() / self._num_sample
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # print('self_att', 'all_input', all_input.size(), 'input_norm', input_norm.size(), 'dec_mask', dec_mask.size())
        ...
        # print('context_attn', 'memory_bank', memory_bank.size(), 'query_norm', query_norm.size(), 'src_pad_mask', src_pad_mask.size())
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(
        ...
        # print(query.size(), key.size(), scores.size())
...
def build_optim(args, model, checkpoint):
    ...
    if checkpoint is not None:
        ...
        # optim = Optimizer(
        #     args.optim, args.lr, args.max_grad_norm,
        #     lr_decay=0.9, start_decay_steps=16737, decay_steps=args.decay_steps,
        #     beta1=args.beta1, beta2=args.beta2,
        #     decay_method='noam_',
        #     warmup_steps=args.warmup_steps)
        # optim._step = 16737

    print('step', optim._step)
...
def build_optim_bert(args, model, checkpoint, search_model_name='bert.model'):
    """ Build optimizer
        search_model_name: bert.model or vq_vae
    """
    ...
    params = [(n, p) for n, p in list(model.named_parameters()) if n.startswith(args.search_model_name)]
    optim.set_parameters(params)

    print('{} step: {}'.format(args.search_model_name, optim._step))
    print('{} params: {}'.format(args.search_model_name, len(params)))

    return optim


def build_optim_not_bert(args, model, checkpoint, search_model_name='bert.model'):
    """ Build optimizer
        search_model_name: bert.model or vq_vae
    """
    ...
    if checkpoint is not None:
        ...
        optim = Optimizer(
            args.optim, args.lr_not_bert, args.max_grad_norm,
            beta1=args.beta1, beta2=args.beta2,
            decay_method='noam',
            warmup_steps=args.warmup_steps_not_bert)

    params = [(n, p) for n, p in list(model.named_parameters()) if not n.startswith(args.search_model_name)]  # and p.requires_grad
    optim.set_parameters(params)

    print('non {} step: {}'.format(args.search_model_name, optim._step))
    print('non {} params: {}'.format(args.search_model_name, len(params)))
