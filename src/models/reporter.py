...
class Statistics(object):
    ...
    * elapsed time
    ...
    def __init__(self, loss=0, n_words=0, n_correct=0, loss_extra=None):
        self.loss = loss
    ...
    def update(self, stat, update_n_src_words=False):
        ...
        if self.loss_extra is not None and stat is not None:
            self.loss_extra += stat.loss_extra
        else:
            self.loss_extra = None
    ...
    def output(self, step, num_steps, learning_rate, start):
        ...
        if isinstance(learning_rate, list):
            lr1, lr2 = learning_rate
        else:
            lr1, lr2 = learning_rate, 0

        loss2 = self.extra_loss()
        if loss2 is None:
            loss2 = 0

        logger.info(
            ("Step %2d/%5d; acc: %6.2f; ppl: %5.2f; xent: %4.2f; loss_extra: %4.2f" +
             "lr1: %7.8e; lr2: %7.8e; %3.0f/%3.0f tok/s; %6.0f sec")
            % (step, num_steps,
               self.accuracy(),
               self.ppl(),
               self.xent(),
    ...
    def log_tensorboard(self, prefix, writer, learning_rate, step):
        ...
        if isinstance(learning_rate, list):
            for li, lr in enumerate(learning_rate):
                writer.add_scalar(prefix + "/lr{}".format(li), lr, step)
        else:
            writer.add_scalar(prefix + "/lr", learning_rate, step)
        if self.extra_loss() is not None:
            writer.add_scalar(prefix + "/loss_extra(VQ-VAE)_scaled", self.extra_loss(), step)
